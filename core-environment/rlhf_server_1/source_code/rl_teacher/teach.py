...
def log_event(user_id, domain, event, tep, iterations, experiment_name):
    print("LOGGING EVENT!!")
    # timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    # log_line = f"[{timestamp}] [{event}] User: {user_id}, Domain: {domain}, -tep: {tep}, -i: {iterations}, Experiment: {experiment_name}\n"
...
def main(env=None, user_id=None, args=None):
    ...
    log_event(user_id=args.user_id, domain=args.domain, event="START", tep=args.episodes, iterations=args.pretrain_iters, experiment_name=experiment_name)
    ...
    if not args.force_new_reward_model:
        ...
        if reward_model.loaded:
            reward_model.clip_manager.free_memory()
    if training and not reward_model.loaded: 
    ...
    log_event(user_id=user_id_arg, domain=domain, event="FINISH", tep=args.episodes, iterations=1000000, experiment_name=experiment_name)
...
def run_server(args=None):
    ...
    if args.port:
        ...
        PORT=9997
    HOST="0.0.0.0"
    print(os.environ.get('POSTGRES_DB', 'Not obtained.'))
