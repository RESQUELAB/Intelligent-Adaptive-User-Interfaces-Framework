...
class Node:
    ...
    def __init__(self, game, done, parent, observation, action_index, depth=0, maxD=4):
    ...
    def create_child(self):
        ...
        for action, game in zip(actions, games):
            ...
            depth = self.D + 1
            child[action] = Node(game, done, self, observation, action, depth=depth)
    ...
    def next(self):
        ...
        # max_T = max(node.T for node in child.values())
        # for node in child.values():
        #     print("This is the REWARD value::: ", node.T, "and max: ", max_T)
       
        max_children = [ c for a,c in child.items() if c.N == max_N ]
        # max_children = [ c for a,c in child.items() if c.T == max_T ]
